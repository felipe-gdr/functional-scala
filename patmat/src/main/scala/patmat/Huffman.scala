...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def addToList(freqList: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = freqList match {
      case List()  => acc
      case x :: xs => Leaf(x._1, x._2) :: addToList(xs, acc)
    }

    addToList(freqs, List()).sortBy(l => l.weight)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case cd1 :: cd2 :: cds => {
      (makeCodeTree(cd1, cd2) :: cds).sortWith((t1, t2) => weight(t1) < weight(t2))
    }
   ...
   */
  def until(isSingleton: List[CodeTree] => Boolean, combinator: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
